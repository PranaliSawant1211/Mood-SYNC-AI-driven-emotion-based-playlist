...
@app1.route('/funprediction', methods=['POST','GET'])
def funprediction():
    global a
    if(a=="Sad"):
        # For Sad
        model = tf.keras.models.load_model('sad1.h5', custom_objects=SeqSelfAttention.get_custom_objects())
        note_tokenizer  = pickle.load( open( "sad_tokenizer1.p", "rb" ) )
        def generate_from_random(unique_notes, seq_len=50):
            generate = np.random.randint(1,unique_notes,seq_len).tolist()
            print("integer:",generate)
            return generate
            
        def generate_from_one_note(note_tokenizer, new_notes):
        ...
        #         print(note_tokenizer.index_to_notes[ind_note])
        ...
        write_midi_file_from_generated(generate, "sad.mid", start_index=seq_len - 1, fs=7, max_generated = max_generate)
    if(a=="Happy"):
            # For Happy
        model = tf.keras.models.load_model('happy.h5', custom_objects=SeqSelfAttention.get_custom_objects())
        note_tokenizer  = pickle.load( open( "happy_tokenizer.p", "rb" ) )
        def generate_from_random(unique_notes, seq_len=50):
            generate = np.random.randint(1,unique_notes,seq_len).tolist()
            print("integer:",generate)
            return generate
        ...
        generate = generate_from_random(unique_notes, seq_len)
        generate = generate_notes(generate, model, unique_notes, max_generate, seq_len)
        write_midi_file_from_generated(generate, "happy.mid", start_index=seq_len - 1, fs=7, max_generated = max_generate)
...
def result():
    ...
    print("hello1")
    model = tf.keras.models.load_model(MODELPATH, custom_objects={'GlorotUniform': glorot_uniform()})
    print("hel")
    image = cv2.imread('detect1.jpg')
    ...
    print(cropped_img)
    print("hello4")
    image1 = cv2.normalize(cropped_img, cropped_img, alpha=0, beta=1, norm_type=cv2.NORM_L2, dtype=cv2.CV_32F)
    
    prediction = model.predict(image1)
    print("hr")
    print(prediction)
    a = emotion_dict[int(np.argmax(prediction))]
    print(a)
